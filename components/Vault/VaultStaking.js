...
const RlpMinting = () => {
    ...
    useEffect(() => {
        console.log(formatting.getTokenAmount(router.estimatedRlpAmount, 0, 4));
        const text =
            `estimated rLP minted: ${formatting.getTokenAmount(router.estimatedRlpAmount, 0, 4)} ` + `rLP ➔ Gas cost: ${formatting.getTokenAmount(router.gasEstimation, 0, 4)} ETH`;
    ...
    return (
        <div>
            <ul className="list-disc list-inside py-4 md:py-8">
                <li>Deposit Ethereum to buy new rLP tokens</li>
...
const VaultDeposit = ({ token }) => {
    ...
    const renderBuyButton = token => {
        if (token === "rLP") {
            return (
                <DeltaButton className="flex-1 md:flex-grow-0" onClick={() => setDepositAction(t => !t)} grayLook={depositAction}>
                    Buy
                </DeltaButton>
            );
        }
        ...
        return (
            <a className="flex-1 md:flex-grow-0" target="_blank" href="https://app.uniswap.org/#/swap" rel="noopener noreferrer">
        ...
        <div>
            <DeltaPanel className="flex items-center text-center flex-wrap">
                <div className="flex border border-black p-1 flex-grow md:flex-none">
                    <DeltaButton className="flex-1 mr-2 md:flex-grow-0" onClick={() => setDepositAction(t => !t)} grayLook={!depositAction}>
                        Stake
                    </DeltaButton>
...
const CreateWithdrawalContractContent = ({ token }) => {
    ...
    return (
        <DeltaPanel>
            ...
            <div>
                <ProgressBarDiamonds small value={10} maxValue={10} />
            </div>
    ...
    return (
        <div className="my-6">
            ...
            <div className="flex p-1 flex-grow md:flex-none">
                ...
                <DeltaButton className="flex-1 md:flex-grow-0" onClick={seeWithdrawingContract}>
                    Show All Contracts
                </DeltaButton>
            </div>
        </div>
...
const VaultWithdraw = ({ token }) => {
    ...
    const renderButtons = () => {
        if (token === "delta") {
            return (
                <>
                    <DeltaButton className="flex-1 mr-2 md:flex-grow-0" onClick={() => setTokenToWithdraw("eth")} grayLook={tokenToWithdraw !== "eth"}>
                        Ethereum
                    </DeltaButton>
                    <DeltaButton className="flex-1 md:flex-grow-0" onClick={() => setTokenToWithdraw("delta")} grayLook={tokenToWithdraw !== "delta"}>
                        Delta
                    </DeltaButton>
                </>
            );
        ...
        return (
            <>
                ...
                <DeltaButton className="flex-1 mr-2 md:flex-grow-0" onClick={() => setTokenToWithdraw("delta")} grayLook={tokenToWithdraw !== "delta"}>
                    Delta
                </DeltaButton>
                <DeltaButton className="flex-1 md:flex-grow-0" onClick={() => setTokenToWithdraw("rlp")} grayLook={tokenToWithdraw !== "rlp"}>
                    rLP
                </DeltaButton>
            </>
        );
    ...
    return (
        <div>
            <DeltaPanel className="flex items-center text-center flex-wrap">
                <div className="flex border border-black p-1 flex-grow md:flex-none">{renderButtons()}</div>
            </DeltaPanel>
            <DeltaPanel>{renderContent(tokenToWithdraw)}</DeltaPanel>
        </div>
    );
};
...
const VaultStaking = ({ token, className = "" }) => {
    ...
    return (
        <DeltaPanel className={`pt-2 border-t-2 mt-4 border-dashed border-black ${className}`}>
            ...
            <div className="mt-4 md:mt-1">{depositAction ? <VaultDeposit token={token} /> : <VaultWithdraw token={token} />}</div>
        </DeltaPanel>
    );
};
