...
const NavBar = () => {
  ...
  const renderMenuItems = () => {
    ...
    return <>
      ...
      <a href="/faq" className={getStyle('/faq')}>FAQ</a>
    </>;
  };

  const renderTokenIcon = ({ tokenLogo, onClick }) => {
    return (
      <div onClick={onClick} className="hidden sm:inline-block rounded-md bg-dark-900 hover:bg-dark-800 p-0.5 cursor-pointer" style={{ width: '36px', height: '36px', background: 'white', display: 'flex', placeItems: 'center', justifyContent: 'center' }}>
