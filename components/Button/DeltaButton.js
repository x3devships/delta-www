...
const DeltaButton = ({ children, className = "", labelBottom, hidePlus, secondaryLook, grayLook, ...props }) => {
        ...
        return (
            <div className={className}>
                <div className="border border-black w-full md:w-max text-black p-0.5">
                    <Button className="w-full md:w-max bg-gray-300 text-black p-5 rounded-none py-4 flex-grow" {...props}>
                        <div className="text-left flex-grow">{children}</div>
    ...
    return (
        <div className={className}>
            <Button className={`w-full md:w-max ${grayLook ? "bg-gray-300 text-gray-500" : "bg-black text-white"} p-5 rounded-none py-4 flex-grow`} {...props}>
                <div className="text-left flex-grow">{children}</div>
